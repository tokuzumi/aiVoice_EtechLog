...
import { AiVoiceProvider, useAiVoice } from './providers/AiVoiceProvider';
import { Widget } from './components/Widget';

function AppContent() {
    const { messages, isLive, status, connect, disconnect, sendMessage } = useAiVoice();
...
export default function App() {
    const wsUrl = import.meta.env.VITE_AGENT_API_URL || "http://localhost:8085";
    const clientName = import.meta.env.VITE_INSTANCE_CLIENT_NAME || "Default";

    return (
        <AiVoiceProvider agentApiUrl={wsUrl} clientName={clientName}>
            <AppContent />
        </AiVoiceProvider>
    );
}
